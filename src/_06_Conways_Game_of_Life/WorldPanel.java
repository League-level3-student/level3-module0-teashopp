...
public class WorldPanel extends JPanel implements MouseListener, ActionListener {
	...
	public void randomizeCells() {
		...
		for (int i = 0; i < cell2d.length; i++) {
			for (int j = 0; j < cell2d.length; j++) {
				...
				int boolgenint = boolgen.nextInt(2 - 1) + 1;
				if (boolgenint == 1) {
					cell2d[i][j].isAlive = true;
				} else if (boolgenint == 2) {
	...
	public void step() {
		...
		for (int i = 0; i < cell2d.length; i++) {
			for (int j = 0; j < cell2d.length; j++) {
				livingNeighbors[i][j] = getLivingNeighbors(cell2d, i, j);
			}
		}
	...
	public void mousePressed(MouseEvent e) {
		...
		int ivalue = e.getX() / cellSize;
		int jvalue = e.getY() / cellSize;
		
		if (cell2d[ivalue][jvalue].isAlive) {
			cell2d[ivalue][jvalue].isAlive = false;
		} else if (!cell2d[ivalue][jvalue].isAlive) {
			cell2d[ivalue][jvalue].isAlive = true;
		}
		repaint();
	}
